...
class Plugin {
	...
	public static function boot(): void {
		...
		// class (e.g. a hotfix shipped before Channel B classes land in the
		// public repo) must still load cleanly.
	 ...
	 *
	 * @param bool $allow_plugins_list Also allow the Plugins list screen.
	 */
	private static function is_own_screen( bool $allow_plugins_list = false ): bool {
		...
		if ( $allow_plugins_list && 'plugins' === $screen->base ) {
			return true;
		}
		return false !== strpos( (string) $screen->id, Admin::PAGE_SLUG );
	}
	...
	public static function maybe_review_prompt(): void {
		...
		<div class="notice is-dismissible astroway-review-prompt" data-astroway-dismiss="<?php echo esc_url( $dismiss_url ); ?>">
			...
			<div class="arp-mark"><img src="<?php echo esc_url( ASTROWAY_WP_PLUGIN_URL . 'assets/img/logo-owl-moon.png' ); ?>" alt="" width="46" height="46"></div>
			<div class="arp-text">
	 ...
	 * is exhausted. The probe below sets `astroway_rate_limit_hit_at` when this
	 * server's own quota runs out; we display the notice for 24h after the most
	 * recent hit, dismissible per-user. Visitors are unaffected either way:
	 * their iframes count against their own IP quota.
	 */
	public static function maybe_rate_limit_notice(): void {
		...
		// The probe lives here now: it measures this server's quota, which is
		// what the site owner needs to know, and it costs one request per 5 min
		// instead of one per front-end render.
		PublicClient::rate_limit_active();
